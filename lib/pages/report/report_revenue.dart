...
class ReportRevenue extends StatelessWidget {
  ...
  const ReportRevenue({super.key, this.showProfit = false});
  ...
  LineChartData mainData(Map<DateTime, List<PenjualanModel>> data) {
    ...
    for (var i in data.entries) {
      double val = showProfit
          ? i.value.fold(0.0, (p, c) => p + (c.totalHarga - c.items.fold(0.0, (p, c) => p + c.hargaDasar! * c.quantity!)))
          : i.value.fold(0.0, (p, c) => p + c.totalHarga);
      if (val > maxVal) {
        maxVal = val;
      }
    }
    double calculatedMaxY = maxVal > 0 ? maxVal * 1.2 : 1000.0;
